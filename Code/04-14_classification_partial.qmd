...
Since our outcome is a class (and not a number), classification performance metrics need to identify the agreement between the **class predicted** and the **class observed**.   These are not used for regression models.  
...
weather_class  <- weather %>%
  ...
  dplyr::select(year, site,
                strength_gtex,
                strength_gtex_class,
                everything())   #moving around the columns
...
weather_split <- initial_split(weather_class, 
                               prop = .7,
                               strata = strength_gtex_class) #the stratified variable we want to use in our data split
...
#brier class the lower the better
```
...
best_accuracy
#bc we used quantiles to make the classes, we have balanced classes. So, our data is equally spaced. Cannot use accuracy for unbalanced data.
```
mtry: 35  
trees: 149  
...
```{r final_fit}
final_fit <- last_fit(final_spec, #this function takes your best hyperparameter values and now runs them on the entire training set. In training, we did training on separate folds. Up until this point, we have only used folds, not the entire dataset. 
                weather_recipe,
                split = weather_split)

final_fit %>%
  collect_predictions() #211 is the test set, thats why there are 211 rows

#shows the probabilities that lead to the classification in the .pred_class column. The model does not predict correctly all the time. How can we check, without RMSE and R2 (which are for numerical variables)
```
...
final_fit %>%
  collect_predictions() %>%
  conf_mat(truth = strength_gtex_class, #what our actual classes were
           estimate = .pred_class) %>% #what the model predicted
  autoplot(type = "heatmap") #all of the values in the matrix add up to 211. Majority of the observations should be on the diagonal.
```

Metrics on the **test set**:
```{r}
final_fit %>%
  collect_metrics()  #accuracy is 0.49. roc_auc the higher to one is better. Brier class the lower the better.
```
Let's hand-calculate accuracy:  
That's how many rows the model got right divided by all observations
```{r}
#  Numerator: number of correctly classified (the diagonal)
numerator <- 41+33+31

# Denominator: number of all observations  
denominator <- 24+6+17+20+13+26+41+33+31
...
final_fit %>%
  ...
  roc_curve(strength_gtex_class, #providing observed values probabilities
            .pred_Low,
            .pred_Medium,
            .pred_High) %>%
ggplot(aes(x = 1 - specificity, 
  ...
  #The model is doing better than random for our test set, but doing the best for the "Low" class
```
...
#can report on this data, but don't want to use this to see how good our model works
```
